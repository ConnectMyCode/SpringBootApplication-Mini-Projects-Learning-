Yes. For Spring MVC / Spring Boot controllers, these are the main annotations you should connect with **where the input comes from**.

| Annotation          | Input comes from      | Example                     | What it means                                            |
| ------------------- | --------------------- | --------------------------- | -------------------------------------------------------- |
| `@PathVariable`     | URL path              | `/orders/10`                | Takes a value from `{}` in the URL                       |
| `@RequestParam`     | URL query parameter   | `/orders?id=10`             | Takes a value after `?`                                  |
| `@RequestBody`      | HTTP request body     | `{"name":"Aryan"}`          | Converts JSON/XML body into a Java object                |
| `@RequestHeader`    | HTTP header           | `Authorization: Bearer xyz` | Takes a value from an HTTP header                        |
| `@CookieValue`      | Browser/client cookie | `JSESSIONID=abc123`         | Takes a value from a cookie                              |
| `@ModelAttribute`   | Form/query parameters | `name=Aryan&age=25`         | Binds multiple request values into a Java object         |
| `@RequestPart`      | Multipart request     | File + fields               | Takes one part of `multipart/form-data`, commonly a file |
| `@SessionAttribute` | HTTP session          | `userId` stored in session  | Gets an attribute already stored in the user's session   |

### The 4 you should learn first

For your current Spring Boot learning, focus heavily on these:
...
Think of the HTTP request as having different **places from which Spring can extract data**:
...
```text
      ...
      URL            BODY          HEADERS
       ...
       │              └──────────────── @RequestBody
       │
       ├── /orders/{id} ─────────────── @PathVariable
       │
       └── ?id=10 ──────────────────── @RequestParam
```

This is the key distinction: **the annotations don't all process JSON**. They tell Spring **where to obtain the controller method's input from**.

--------------------------------------------------------------------------------------------------------------------------------------------------------------


![From-HttpRequestToJsonObject.png](From-HttpRequestToJsonObject.png)


![Spring_WEBMVC_model_.png](Spring_WEBMVC_model_.png)


![WEBMVC_FLOW.png](WEBMVC_FLOW.png)
